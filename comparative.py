...
def get_total_count_youtube(client, db, celeb_name):
...
@handle_mongodb_connection
def get_total_count_reddit(client,db,subreddit):
...
def start_influence():
    ...
    for celeb in celeb_name:
        youtube_like_count.append(get_total_count_youtube(celeb))
    print(youtube_like_count)
    
    reddit_count = []
    for subreddit in celebrities:
        reddit_count.append(get_total_count_reddit(subreddit))
    # print(type(reddit_count))
...
def plot_data(data, source):
    ...
    plt.title(f'Celebrity Influence {source}')
    t = time.time()
    plt.savefig(f'plots/comparativePlots/plot_{source}_{time.ctime(t)}.png')
    plt.show()
